...
AFKLoop:
    loop{
		 ;gosub, Menu
		 gosub, PressX
        credit := Round((racecounter * 70 + cleanrace * 35)/1000, 3)
        currenttime := A_TickCount
        income := Round(credit / (currenttime - starttime) * 3600000, 3)
        ToolTip, R %racecounter% CR %cleanrace% Cr %credit%M Cr/H %income%M, 250, 50, screen
