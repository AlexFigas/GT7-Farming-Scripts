...
; GUI
Gui, New, -MaximizeBox -Resize, ClubmanPlus 1.2-SNAPSHOT
Gui, Margin, 10, 10
; GUI: Credits
...
Gui, Add, Text, xp y+5 wp vRunningTime, Running Time: 00:00:00
; GUI: Options
Gui, Add, GroupBox, x12 w315 h45, % "Pixel detect"
Gui, Add, Radio, xp+10 yp+20 w150 Checked vPixelType1, % "Type 1"
Gui, Add, Radio, xp+150 yp wp vPixelType2, % "Type 2"
; GUI: Buttons
    ...
    Loop {
        ...
        Loop {
            while (IsColor(hwnd, 0xFFFFFF, (PixelType1) ? (960 / 218) : (640 / 155), (PixelType1) ? (540 / 490) : (360 / 320), 6, 50)) { ; top-right tire wear indicator
                ...
                if (!do_steering && IsColor(hwnd, 0xC34F4F, 960 / 834, 540 / 18, 5, 30)) {
                    OutputDebug % "Clubman> Race: Found car on steering sector. Steering..."
                    controller.Axes.LX.SetState(80)
                    do_steering := !do_steering
                }
                else if (do_steering && IsColor(hwnd, 0xC34F4F, 960 / 869, 540 / 20, 5, 30)) {
...
Reset:
    ...
    ; Gosub, GrabWindow
    ; PixelSearch(960 / 834, 540 / 18, 4, hwnd, "prueba")
    ; PixelSearch(960 / 869, 540 / 20, 4, hwnd, "prueba1")
    ; C34F4F
return
